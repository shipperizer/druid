...
if ! docker buildx inspect skaffold-builder-druid >/dev/null 2>&1; then
  docker buildx create --name skaffold-builder-druid --platform $platforms
fi
...
docker buildx build --builder skaffold-builder-druid --tag $IMAGE $args -f $dockerfile "$BUILD_CONTEXT"
